...
During the setup or after restoring a backup to **Rokku**:
1. In <nav to="data-and-storage">, double-check your specified [Storage location](/docs/faq/storage) that **Rokku** has access to.
...
Cross device sync in **Rokku** is not currently available, but users can use
...
1. On your second device, download the latest backup from your cloud service to restore into **Rokku**.
...
All **Rokku** (and **Tachiyomi**) [forks](/forks/) support the `.tachibk`/`.proto.gz` format to backup/restore your library.
...
- These fork-specific settings will not be restored in **Rokku** and will be lost after restoring.
...
Be aware of these limitations when dealing with backups in different **Rokku** and **Tachiyomi** forks.
