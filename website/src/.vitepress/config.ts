  ...
  transformPageData: async (pageData) => {
    if (pageData.filePath === 'changelogs/[tag].md') {
      ...
      if (tag) {
        ...
        if (!publishedAt) {
          try {
            const { data } = await octokit.repos.getReleaseByTag({ owner: 'rokku-app', repo: 'rokku', tag })
        ...
        const desc = prettyDate
          ? `Changelog for Rokku ${versionLabel}, released on ${prettyDate}`
          : `Changelog for Rokku ${versionLabel}`
